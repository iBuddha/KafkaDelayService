...
import kafka.delay.message.utils.{DelayServiceConfig, SystemTime, _}
...
/**
  *
  * 从timer对应的message queue里poll消息，然后发送给MessageConsumer, 由MessageConsumer发给Producer，再发给实际用户从中拉取超时
  * 消息的topic。
  *
  * MessageConsumer会整个过程最终结果报告给这个Actor。当其中的任何环节出现问题时，TimerConsumer都会不停地重试。
  * 在重试的过程中，它不再从timer拉取超时的消息
  *
  *
  * 它会尽量从timer中拉取已超时的消息，直至下面任何一个条件达到：
  * 1. 已经暂存的message meta超过了最大的暂存时间，以至于可能导致最终发送到用户的时间延迟过久
  * 但是，如果在拉取到某个消息时，此消息的超时时间和当前时间的差值超过了一个限度，就会开启dynamic batch模式。此时，会拉取当
  * 前可以拉取的所有消息，而忽略暂存时间的限制。即牺牲送到时间，以期通过增加batch读取的效率，挽回之前处理效率的不足。
  *
  * 比如，在消息超时之前就从timer里读出来。或者在加到timer之前修改absolute time, 让它提前一段时间
  *
  */
class NonAskTimerConsumer(timerConsumerConfig: NonAskTimerConsumerConfig)
  extends Actor with ActorLogging {
  private implicit val executionContext = context.dispatcher

  import NonAskTimerConsumer._

  private val expiredMessageQueue = timerConsumerConfig.expiredMessageQueue
  private val timer = timerConsumerConfig.timer
  private val messageConsumer = timerConsumerConfig.messageConsumer
  ...
  //  private val maxStashSize = config.maxStashSize
  ...
  //  private var preFetched: Option[OffsetBatchList] = None
  ...
  private val checkInterval = timerConsumerConfig.expiredSendTimeoutMs millis
  ...
  private var warmer: Option[ActorRef] = None
  ...
  override def receive: Receive = {
    ...
    case WarmerRef(warmerRef) => warmer = Some(warmerRef)
  ...
  private def prefetchFromTimer(max: Int): Option[OffsetBatchList] = {
    val now = SystemTime.hiResClockMs
    var completed = false
    val buffer = mutable.ListBuffer.empty[DelayMessageMeta]
    while (!completed && buffer.size < max) {
      val expired = expiredMessageQueue.poll()
      if (logTimeDiffEnable) {
        logTimeDiffMs(expired, now)
      }
      if (expired == null) {
        completed = true
      } else {
    ...
    if (buffer.isEmpty) {
      ...
      val rolled = roll(buffer.toList, maxBatchDistance, maxBatchRange, maxBatchSizeHistorically)
      Some(OffsetBatchList(rolled.toList))
    }
  }

  private var lastPeekTime = 0L
  private var lastPeekedBuckets = Map.empty[Long, SortedSet[DelayMessageMeta]]
  private val minPeekBucketNum = 3
  ...
  private def mayBeSendPeeked(maxNumber: Int): Unit = {
    ...
    if(currentTime != lastPeekTime) {
      ...
      if (!peeked.overflowed.isEmpty) {
        ...
        warmer.foreach { w => w ! PreFetchBatchListRequest(OffsetBatchList(rolled.toList)) }
      }
      peeked.buckets.foreach { bucket =>
        val newlyPeeked =
          if (!lastPeekedBuckets.contains(bucket.bucketTime)) {
            ...
            bucket.messages -- lastPeekedBuckets(bucket.bucketTime)
          } else {
            SortedSet.empty[DelayMessageMeta]
          }
        val rolled = roll(newlyPeeked.toList, maxBatchDistance, maxBatchRange, maxBatchSizeHistorically)
        warmer.foreach { w => w ! PreFetchBatchListRequest(OffsetBatchList(rolled.toList)) }
      }
...
object NonAskTimerConsumer {
  ...
  case class NonAskTimerConsumerConfig(metaStorageCreator: () => MetaStorage,
